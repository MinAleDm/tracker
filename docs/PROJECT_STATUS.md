...
Последнее обновление: 2026-08-27. Состояние: `main` после `1.1.0`.

## Готовность

| Область | Состояние | Примечание |
| --- | --- | --- |
| Основные task flows | Готово | Projects, tasks, comments, activity, shared filters/views, board, analytics |
| Web UI | Готово | shadcn/ui, responsive shell, keyboard navigation, light/dark themes, explicit states |
...
| Dependency security | Готово | На 2026-08-27 high audit: 0 известных уязвимостей |
...
| Browser E2E/a11y | Частично | Production smoke и desktop/mobile visual audit выполнены; постоянный regression suite отсутствует |
...
- Frontend переведён на общий shadcn/ui-слой; удалены повторяющиеся KPI, inline-формы в каждой строке и
  условный project pulse, который не подтверждался доменными данными.
- Edge auth limits разделяют login/invitation и session refresh/logout, поэтому обычные перезагрузки не
  приводят к ложному разлогину; API rate limits остаются обязательными.
- Web-контейнер больше не публикует отдельный browser port в Compose; единственная точка входа — Nginx.
...
6. Зафиксировать Playwright smoke/visual/a11y suite в CI и провести независимый security review.
